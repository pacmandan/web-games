defmodule WebGamesWeb.PageHTML do
  ...
  def game_card(%{enabled: true} = assigns) do
    ~H"""
    <a href={@href} class="block float-left max-w-xs border-white border-4 p-4 m-4 w-[275px] rounded-lg bg-slate-500 hover:bg-slate-400">
  ...
  def game_card(%{enabled: false} = assigns) do
    ~H"""
    <div class="block float-left max-w-xs border-gray border-4 p-4 m-4 w-[275px] rounded-lg bg-gray-500">
